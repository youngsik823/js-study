...
alertUserInfo(u => console.log(`나는 ${u.userName}님 이시다!!`));

// 배열 고차 함수
// forEach() : 배열의 요소를 반복하면서 하나씩 꺼내줌
// for of 안쓰고 forEach쓰면된다.
...
console.log('====================================');
const mappedApples = appleBasket.filter(apple => apple.color ==='green' && apple.sweet >= 9).map(apple => `이 사과는 ${apple.color}색이며 당도가 ${apple.sweet}입니다.`);
...
const mappedhobby = userList.filter(hobbies => hobbies.address === '서울').map(hobbies => hobbies.hobbys[0]);
